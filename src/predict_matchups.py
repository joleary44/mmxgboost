...
def load_selected_model(model_name: str):
    ...
    if model_name == "logistic":
        try:
            return load_model_and_metadata(config.LOGISTIC_MODEL_PATH, config.LOGISTIC_METADATA_PATH)
        except Exception:
            # Cloud environments can fail to unpickle saved artifacts if package
            # versions differ. Re-training the lightweight logistic baseline is
            # fast, so we rebuild it automatically as a friendly fallback.
