...
def available_models() -> list[str]:
    """Return saved models that exist on disk."""
    models: list[str] = []
    if config.LOGISTIC_MODEL_PATH.exists():
        models.append("logistic")
    if config.XGBOOST_MODEL_PATH.exists():
...
def app_main() -> None:
    ...
    if not models:
        st.error(
            "No saved models were found. Train a model first with "
            "`python -m src.train_logistic` or `python -m src.train_xgboost`."
        )
        st.stop()
